...
import FormPage from 'pages/Jobs/components/FormPage';

export default function EditDialog({ job, monthYear, dates }) {
  ...
  async function handleEdit(values, actions) {
    ...
    try {
      ...
      if (newJob.id) {
        ...
        actions.resetForm();
        mutate(
          `/api/v1/users/${job.user_id}/user-jobs?&my=${monthYear}&start=${dates[0]}&end=${dates[1]}`
        );
        handleClose();
      } else {
